...
void VSTPluginInstance::prepareForProcessing(float sampleRate, int blockSize)
{
	...
	//int effVendorSpecified = 0x32;
	//char x[255];
	//effect->dispatcher(effect, 44, 0, 0, NULL, 0.0f);
	//effect->dispatcher(effect, 77, 0, 0, NULL, 0.0f);
...
void VSTPluginInstance::startProcessing()
{
	...
	//effect->dispatcher(effect, effCanDo, 0, 0, (void*) "bypass", 0);
